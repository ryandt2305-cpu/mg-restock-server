-- Add 'tool' as a valid shop_type across all restock tables, functions, and views.

-- ============================================================
-- 1) restock_events — relax shop_type CHECK
-- ============================================================

ALTER TABLE public.restock_events
  DROP CONSTRAINT IF EXISTS restock_events_shop_type_check;
ALTER TABLE public.restock_events
  ADD CONSTRAINT restock_events_shop_type_check
    CHECK (shop_type IN ('seed','egg','decor','tool'));

-- ============================================================
-- 2) restock_history — relax shop_type CHECK
-- ============================================================

ALTER TABLE public.restock_history
  DROP CONSTRAINT IF EXISTS restock_history_shop_type_check;
ALTER TABLE public.restock_history
  ADD CONSTRAINT restock_history_shop_type_check
    CHECK (shop_type IN ('seed','egg','decor','tool'));

-- ============================================================
-- 3) shop_cycle_stats — relax CHECK only (do NOT drop PK)
-- ============================================================

ALTER TABLE public.shop_cycle_stats
  DROP CONSTRAINT IF EXISTS shop_cycle_stats_shop_type_check;
ALTER TABLE public.shop_cycle_stats
  ADD CONSTRAINT shop_cycle_stats_shop_type_check
    CHECK (shop_type IN ('seed','egg','decor','tool'));

INSERT INTO public.shop_cycle_stats (shop_type, total_cycles, last_cycle_ts)
  VALUES ('tool', 0, NULL) ON CONFLICT DO NOTHING;

-- ============================================================
-- 4) restock_item_events — relax shop_type CHECK
-- ============================================================

ALTER TABLE public.restock_item_events
  DROP CONSTRAINT IF EXISTS restock_item_events_shop_type_check;
ALTER TABLE public.restock_item_events
  ADD CONSTRAINT restock_item_events_shop_type_check
    CHECK (shop_type IN ('seed','egg','decor','tool'));

-- ============================================================
-- 5) restock_snap_timestamp — add tool branch
-- ============================================================
...
BEGIN
  IF    p_shop_type = 'seed'  THEN interval_ms := 300000;
  ELSIF p_shop_type = 'egg'   THEN interval_ms := 900000;
  ELSIF p_shop_type = 'decor' THEN interval_ms := 3600000;
  ELSIF p_shop_type = 'tool'  THEN interval_ms := 600000;
  ELSE                             interval_ms := 300000;
  END IF;
...
-- ============================================================
-- 6) ingest_restock_history — accept 'tool', add tool interval
-- ============================================================
...
-- ============================================================
-- 7) Prediction view — add WHEN 'tool' THEN 600000 to cycle_ms
-- ============================================================
...
-- ============================================================
-- 8) Rebuild path -- use correct tool interval (600000ms)
-- ============================================================
