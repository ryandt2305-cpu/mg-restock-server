-- Restore celestial micro-gap filter in ingest_restock_history and rebuild_restock_history.
--
-- The tool-shop migration (20260409000001) re-created both functions from the
-- "no-pity" version (20260408000006) which had intentionally removed celestial
-- pity ramps but also dropped the micro-gap noise filter as collateral.
--
-- This migration restores the >= 21600000 ms (6 h) filter for celestial items
-- in both incremental ingest and full rebuild paths. It does NOT restore
-- pity ramps or probability caps — those were intentionally removed.

-- ============================================================
-- 1) ingest_restock_history — restore celestial micro-gap filter
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    IF v_prev_last_seen IS NOT NULL AND v_snapped_ts > v_prev_last_seen THEN
      ...
      -- RESTORED: filter out celestial micro-gaps (< 6h) for robust median.
...
-- ============================================================
-- 2) rebuild_restock_history — restore celestial micro-gap filter
-- ============================================================
...
BEGIN
  ...
  -- RESTORED: celestial micro-gap filter (>= 6h) in interval stats.
  WITH intervals AS (
    SELECT
      item_id,
      shop_type,
