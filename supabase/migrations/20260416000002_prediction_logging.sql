-- Add prediction snapshot column to restock_item_events and update
-- the get_item_restock_events RPC to return it.
--
-- For every new restock event, we snapshot the estimated_next_timestamp
-- that was active in restock_predictions at that moment. This creates a
-- verifiable audit trail: for every event we know what was predicted vs
-- what actually happened.

-- ============================================================
-- 1) Add predicted_next_ms column
-- ============================================================
...
-- ============================================================
-- 2) Update ingest_restock_history to snapshot predictions
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Snapshot the prediction that was active when this event arrived.
    SELECT rp.estimated_next_timestamp
      INTO v_predicted_next
    FROM restock_predictions rp
    WHERE rp.item_id = v_item_id
      AND rp.shop_type = v_shop_type;
...
-- ============================================================
-- 3) Update get_item_restock_events RPC to return predicted_next_ms
-- ============================================================

-- Must drop first — cannot change return type with CREATE OR REPLACE.
DROP FUNCTION IF EXISTS public.get_item_restock_events(text, text, integer);
...
AS $$
  ...
  SELECT e.timestamp AS event_ts, e.quantity, e.predicted_next_ms
