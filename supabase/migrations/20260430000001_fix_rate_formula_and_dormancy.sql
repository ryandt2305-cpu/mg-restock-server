-- Fix restock prediction accuracy: lifespan-based rate formula + dormancy detection.
--
-- Bug 1 (CRITICAL): appearance_rate divides total_occurrences by total_cycles
-- (all cycles since tracking began), but items added mid-tracking get their
-- rate deflated because they weren't visible during early cycles.
-- Fix: divide by cycles within the item's own lifespan (first_seen to last_seen).
--
-- Bug 2: No dormancy detection. Seasonal/removed items show active predictions.
-- Fix: detect when elapsed_ms > 20x expected interval (min 7 days) and suppress.
--
-- Issue 3: weather_events and restock_item_events have RLS but no anon SELECT policy.
-- Issue 4: restock_item_events CHECK already includes 'tool' (no-op safety).

-- ============================================================
-- Section 1: RLS policies for weather_events + restock_item_events
-- ============================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'weather_events'
      AND policyname = 'anon_select_weather'
  ) THEN
    CREATE POLICY "anon_select_weather"
      ON public.weather_events
      FOR SELECT TO anon
      USING (true);
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'restock_item_events'
      AND policyname = 'anon_select_restock_item'
  ) THEN
    CREATE POLICY "anon_select_restock_item"
      ON public.restock_item_events
      FOR SELECT TO anon
      USING (true);
  END IF;
END
...
-- ============================================================
-- Section 2: CHECK constraint safety (tool already included, idempotent)
-- ============================================================

ALTER TABLE restock_item_events DROP CONSTRAINT IF EXISTS restock_item_events_shop_type_check;
ALTER TABLE restock_item_events ADD CONSTRAINT restock_item_events_shop_type_check
  CHECK (shop_type IN ('seed', 'egg', 'decor', 'tool'));

-- ============================================================
-- Section 3: Fix ingest_restock_history — lifespan-based rate + dedup guard
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Read current state from restock_history (moved before item_events INSERT
    -- so we can dedup and compute predicted_next without a second SELECT).
    ...
    -- Dedup guard: skip if this cycle was already ingested for this item.
    IF v_prev_last_seen IS NOT NULL AND v_item_snapped_ts = v_prev_last_seen THEN
      CONTINUE;
    END IF;

    -- Compute predicted_next for item event logging.
    ...
    -- Lifespan-based appearance rate: divide occurrences by cycles within
    -- this item's own observation window, not all cycles since tracking began.
...
-- ============================================================
-- Section 4: Fix rebuild_restock_history — lifespan-based rate
-- ============================================================
...
BEGIN
  ...
  -- Rate and derived fields — lifespan-based appearance_rate.
  UPDATE restock_history h
  SET appearance_rate = CASE
        ...
        THEN GREATEST(0.0001, LEAST(0.9999, ROUND(
          h.total_occurrences::numeric / GREATEST(1, ROUND((h.last_seen - h.first_seen)::numeric /
            CASE h.shop_type
...
-- ============================================================
-- Section 5: Update restock_predictions view — dormancy + is_dormant
-- ============================================================
...
SELECT
  ...
  p.total_quantity,
  p.total_occurrences,
  -- Estimated next: NULL for dormant items.
  CASE
...
-- ============================================================
-- Section 6: Algorithm version bump
-- ============================================================
...
VALUES (
  ...
  'adaptive-v9-lifespan-rate',
  now(),
  'Lifespan-based appearance_rate (fixes deflation for mid-tracking items). Dormancy detection for seasonal items. RLS anon policies for weather/item events.'
)
...
-- ============================================================
-- Section 7: Full rebuild with corrected rate formula
-- ============================================================
