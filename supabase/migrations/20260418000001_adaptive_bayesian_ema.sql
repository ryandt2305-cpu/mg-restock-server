-- Fix overdue sentinel bug + adaptive Bayesian pseudo_n + EMA intervals.
--
-- Three model improvements in one migration (all touch the same view/functions):
--
-- 1) CRITICAL: Fix overdue sentinel (0.9999 → bounded escalation).
--    When all historical intervals are exceeded, the Kaplan-Meier model was
--    returning emp=0.9999 regardless of item frequency. This inflated seasonal
--    items like Rose (0.95% fallback) to ~40% current_probability (43x overstatement).
--    Fix: cap overdue escalation at 3x fallback_rate.
--
-- 2) Adaptive pseudo_n: scale Bayesian smoothing by data volume.
--    Fixed pseudo_n=8 over-smooths high-data items (Apple: 4800+ occurrences
--    still pulled toward prior). Now: 8→2 as data grows.
--
-- 3) EMA (Exponential Moving Average) intervals for trend detection.
--    Most items show increasing intervals recently (EMA > median).
--    Blends 40% EMA + 60% median for baseline_interval_ms.

-- ============================================================
-- 1) Add EMA column to restock_history
-- ============================================================

ALTER TABLE restock_history ADD COLUMN IF NOT EXISTS ema_interval_ms bigint;

-- ============================================================
-- 2) EMA helper function
-- ============================================================
...
BEGIN
  ...
  ema := intervals[1];
  FOR i IN 2..cardinality(intervals) LOOP
    IF intervals[i] IS NOT NULL AND intervals[i] > 0 THEN
...
-- ============================================================
-- 3) Update ingest_restock_history — maintain EMA incrementally
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Incremental interval window for adaptive median + EMA.
...
-- ============================================================
-- 4) Update rebuild_restock_history — compute EMA from arrays
-- ============================================================
...
BEGIN
  ...
  -- Compute EMA from rebuilt interval arrays.
  UPDATE restock_history
  SET ema_interval_ms = compute_ema_from_array(recent_intervals_ms, 0.15)
...
-- ============================================================
-- 5) Recreate predictions view with all three fixes
-- ============================================================
...
interval_stats AS (
  SELECT
    ...
    -- Blend EMA with median for baseline (40% EMA + 60% median)
    COALESCE(
      CASE
        ...
        ELSE COALESCE(b.ema_interval_ms, b.median_interval_ms)
      END,
...
empirical AS (
  SELECT
    ...
    CASE
      ...
      -- FIXED: bounded overdue escalation instead of flat 0.9999
      ...
      ELSE LEAST(
        ...
        GREATEST(
          0.0001::numeric,
          -- Adaptive pseudo_n in Bayesian smoothing
...
-- ============================================================
-- 6) Bump algorithm version + log to history
-- ============================================================
...
VALUES (
  ...
  'adaptive-v7-bayesian-ema',
  now(),
  'Fix overdue sentinel (0.9999→bounded 3x escalation). Adaptive pseudo_n (8→2 by data volume). EMA intervals (alpha=0.15) blended 40/60 with median for baseline.'
)
...
-- ============================================================
-- 7) Rebuild all data with new model
-- ============================================================
