-- Weather-stratified intervals: per-weather interval tracking.
--
-- Adds a weather_intervals jsonb column to restock_history that buckets
-- intervals by weather at time of restock. Structure:
--   { "Frost": [300000, ...], "Sunny": [...] }
-- Each weather bucket holds up to 20 intervals (newest kept).
--
-- Coverage is limited (~16.7% of historical events have weather_id),
-- so this is informational only — no weather-conditional probability yet.

-- ============================================================
-- 1) Add weather_intervals column
-- ============================================================

ALTER TABLE restock_history
  ADD COLUMN IF NOT EXISTS weather_intervals jsonb DEFAULT '{}'::jsonb;

-- ============================================================
-- 2) Update ingest_restock_history — bucket intervals by weather
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    SELECT h.last_seen, h.recent_intervals_ms, h.ema_interval_ms, h.weather_intervals
      INTO v_prev_last_seen, v_recent_intervals, v_prev_ema, v_weather_intervals
    FROM restock_history h
    WHERE h.item_id = v_item_id
      AND h.shop_type = v_shop_type;
    ...
    IF v_prev_last_seen IS NOT NULL AND v_item_snapped_ts > v_prev_last_seen THEN
      ...
      IF v_current_weather IS NOT NULL AND v_last_interval_ms > 0 THEN
        ...
        -- Cap at v_weather_max_per_bucket per weather (keep newest).
...
-- ============================================================
-- 3) Update rebuild_restock_history — build weather_intervals
-- ============================================================
...
BEGIN
  ...
  -- Only ~16.7% of restock_events have weather_id populated,
  -- so coverage is partial. Intervals without weather data are excluded.
  WITH event_weather AS (
    SELECT
      ...
      d.ts,
      d.qty,
      -- Try restock_events.weather_id first, then weather_events lookup.
      COALESCE(
        d.weather_id,
...
-- ============================================================
-- 4) Update predictions view to expose weather_intervals
-- ============================================================
...
-- ============================================================
-- 5) Rebuild with weather stratification
-- ============================================================
