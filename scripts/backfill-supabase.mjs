...
async function fetchShopTypeRegistry() {
  ...
  const res = await fetch(
    `${SUPABASE_REST_ENDPOINT}/shop_type_registry?is_active=eq.true&select=shop_type`,
  ...
  console.log(`[backfill] Loaded ${rows.length} shop types from registry: ${SHOP_TYPES.join(", ")}`);
}
...
async function loadMgData() {
  ...
  if (toolsRes && toolsRes.ok) {
    ...
    console.warn(`[backfill] /data/tools fetch failed with ${toolsRes.status}; skipping tool validation.`);
  }
...
async function main() {
  ...
  console.log(`Normalized ${normalized.length}/${rawEvents.length} events`);
  if (dedupedCount > 0) {
  ...
  if (rejects.size > 0) {
    ...
    writeJson(path.join(DATA_DIR, "backfill-rejects.json"), {
      totalRejected: Array.from(rejects.values()).reduce((a, b) => a + b, 0),
      top,
    });
    console.log(`Wrote backfill-rejects.json with top ${top.length} rejects`);
  }
